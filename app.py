...
def fit_sheet_to_a4(ws):
    ...
    # データ行（12〜42）にブーストを掛け、全体を正規化して target_h_pt に収める
    # → ヘッダー/フッターの比率を維持しつつデータ行だけ背を高くする
    ...
    weighted_total = sum(header_h) + sum(data_h) * DATA_BOOST + sum(footer_h)
    scale = target_h_pt / weighted_total if weighted_total > 0 else 1.0

    for row_num in range(1, HEADER_END + 1):
        if row_num <= n_rows:
            ws.row_dimensions[row_num].height = row_heights[row_num - 1] * scale

    for row_num in range(DATA_START, DATA_END + 1):
        if row_num <= n_rows:
            ws.row_dimensions[row_num].height = row_heights[row_num - 1] * scale * DATA_BOOST
